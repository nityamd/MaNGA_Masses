...
print('... before the calculation (all spaxels) ...', datetime.datetime.now())
...
for i in range(m,n):
    ...
    measures = galaxy.get_spec_measure(-1,'all')
...
afile = open(r'/Volumes/500GB/nmd299/Data/mpl8/all_spaxels_c.pkl', 'wb')
pickle.dump(sample_ifu,afile)

print('... before the calculation (3 arcsec) ...', datetime.datetime.now())
...
for i in range(m,n):
    start = datetime.datetime.now()
    galaxy = aperture_measurements(fits_files[i],z[i])
    measures = galaxy.get_spec_measure(3,'all')
    sample.append(measures)
    end = datetime.datetime.now()
    elapsed = end - start
    if i%10==0:
...
sample_ifu = np.array(list(zip(plate_ifu[m:n],z[m:n],np.array(sample)[:,0], np.array(sample)[:,1])),
                        dtype = [('plate_ifu', 'U25'),('z', 'f8'), ('hdelta', 'f8'), ('dn4000', 'f8')])
afile = open(r'/Volumes/500GB/nmd299/Data/mpl8/3arcsec_c.pkl', 'wb')
pickle.dump(sample_ifu,afile)
